...
class Server:
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        ...
        assert idx >= 0 and idx <= keys[-1]
        [idxs.append(i) for i in keys if i >= idx and len(idxs) <= page_size]
        data = [dataset[v] for v in idxs[:-1]]
        next_index = idxs[-1] if len(idxs) - page_size == 1 else None
